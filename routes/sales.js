...
router.post("/", async (req, res) => {
  try {
    ...
    const currentIncome = currentSalesData.reduce(
      (acc, sale) => acc + parseFloat(sale.total),
      0
    );
    const currentExpenses = await getSumOfExpensesForCurrentDate();
    const currentNet = currentIncome - currentExpenses;
    const currentCredit = currentSalesData.reduce(
      (acc, sale) => acc + parseFloat(sale.credit),
      0
    );
    const currentComputer = currentSalesData.reduce(
      (acc, sale) => acc + parseFloat(sale.pc?.computer || 0),
      0
    );

    const currentPs4 = currentSalesData.reduce(
      (acc, sale) => acc + parseFloat(sale.ps4),
      0
    );
...
router.post("/add", async (req, res) => {
  ...
  try {
    ...
    if (checkRows.length > 0) {
      ...
      client.release();
      console.log(
        `Sale with transactionid ${sale.transactionid} already exists.`
      );
      return res
        .status(409)
        .json({ error: "Sale with this transactionid already exists" });
    }

    const insertQuery = `
      INSERT INTO sales (transactionid, orders, qty, total, datetime, customer, computer, ps4, subtotal, credit, mode_of_payment, student_discount, discount, load, zombiepromo)
    ...
    const insertValues = [
      ...
      sale.discount,
      sale.pc?.load || 0,
      sale.pc?.zombiePromo || false,
    ];

    const { rows: insertedSaleRows } = await client.query(
      insertQuery,
      insertValues
    );
    ...
    const [baristaResults, utensilsResults, beveragesResults] =
      await Promise.all([
        client.query(
          "SELECT product FROM products WHERE product = ANY($1) AND barista = true",
          [productNames]
        ),
        client.query(
          "SELECT product FROM foods WHERE product = ANY($1) AND utensils = true",
          [productNames]
        ),
        client.query("SELECT product FROM beverage WHERE product = ANY($1)", [
          productNames,
        ]),
      ]);
    ...
    const totalBaristaQuantity = sale.orders
      .filter((order) =>
        baristaResults.rows.some((bp) => bp.product === order.product)
      )
      .reduce((sum, order) => sum + order.quantity, 0);

    const totalUtensilsQuantity = sale.orders
      .filter((order) =>
        utensilsResults.rows.some((up) => up.product === order.product)
      )
      .reduce((sum, order) => sum + order.quantity, 0);

    const totalBeverageQuantity = sale.orders
      .filter((order) =>
        beveragesResults.rows.some((b) => b.product === order.product)
      )
      .reduce((sum, order) => sum + order.quantity, 0);
    ...
    insertedSale.pc = {
      computer: insertedSale.computer || "",
      load: parseFloat(insertedSale.load || 0),
      zombiePromo: insertedSale.zombiepromo || false,
    };

    delete insertedSale.computer;
    delete insertedSale.load;
    delete insertedSale.zombiepromo;

    console.log("Sending inserted sale:", insertedSale);
...
router.post("/date-range", async (req, res) => {
  try {
    ...
    const filteredIncome = rows.reduce(
      (acc, sale) => acc + parseFloat(sale.total),
      0
    );
    const filteredExpenses = await getSumOfExpensesByDateRange(
      startDate,
      endDate
    );
    const filteredNet = filteredIncome - filteredExpenses;
    const filteredCredit = rows.reduce(
      (acc, sale) => acc + parseFloat(sale.credit),
      0
    );
    const filteredComputer = rows.reduce(
      (acc, sale) => acc + parseFloat(sale.computer),
      0
    );
    const filteredFoodAndDrinks = rows.reduce(
      (acc, sale) => acc + parseFloat(sale.subtotal),
      0
    );
...
router.post("/member-sales-today", async (req, res) => {
  try {
    ...
    const totalCredit = salesData.reduce(
      (acc, sale) => acc + parseFloat(sale.credit),
      0
    );
    const totalComputer = salesData.reduce(
      (acc, sale) => acc + parseFloat(sale.computer),
      0
    );
    const totalFoodAndDrinks = salesData.reduce(
      (acc, sale) => acc + parseFloat(sale.subtotal),
      0
    );
  ...
  try {
    ...
    rows.forEach((row) => {
      ...
      row.pc = {
        ...
        zombiePromo: row.zombiepromo || false,
      };

      delete row.computer;
      delete row.load;
      delete row.zombiepromo;
    });
...
async function getSumOfCredits(startDate, endDate) {
  let queryText =
    "SELECT COALESCE(SUM(credit::numeric), 0) AS total_credit FROM sales";
...
async function getSumOfComputers(startDate, endDate) {
  let queryText =
    "SELECT COALESCE(SUM(load::numeric), 0) AS total_computer FROM sales";
...
async function getSumOfPs4(startDate, endDate) {
  let queryText =
    "SELECT COALESCE(SUM(ps4::numeric), 0) AS total_ps4 FROM sales";
...
async function getSumOfFoodAndDrinks(startDate, endDate) {
  let queryText =
    "SELECT COALESCE(SUM(subtotal::numeric), 0) AS total_food_and_drinks FROM sales";
